...
// Ensure `data` is globally accessible and correctly set by fetchPhotographersData
...
const updatePhotographerName = () => {
    const params = new URLSearchParams(window.location.search);
    const photographerId = params.get('id'); // Assumes URL is something like ?id=123
    const photographerNameElement = document.querySelector('.modal_form_name');

    if (typeof data === 'undefined' || !data.photographers) {
        console.error('Data not loaded or photographers array is missing:', data);
        photographerNameElement.textContent = "Photographer not found";
        return;
    }
...
// Call the initialize function once the DOM is fully loaded
